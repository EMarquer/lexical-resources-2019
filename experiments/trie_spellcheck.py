...
def levenshtein_distance(word_1: str, word_2: str) -> int:
    ...
    # this covers:
    # - deletion (delete from word 2);
    # - insertion (delete from word 1);
    # - replacement (delete from both and different last character);
    # - no edit (delete from both and same last character).
    return min(
        levenshtein_distance(word_1[:-1], word_2) + 1, # edit is removing one character from word_1
        levenshtein_distance(word_1, word_2[:-1]) + 1, # edit is removing one character from word_2
        levenshtein_distance(word_1[:-1], word_2[:-1]) + ( 
            1 if (word_1[-1] != word_2[-1]) else # edit is changing the current last character
            0) # no edit needed on the last character
